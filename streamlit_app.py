...
def page_google_sheet():
    ...
    sheet_title = st.text_input("Nom du Google Sheet à créer", value=f"{client}_template" if client else "")
...
def page_configuration_ia():
    ...
    st.session_state.model = st.selectbox("Modèle", sheet_manager.get_available_models(st.session_state.provider))
...
def page_generation():
    ...
    for record in records:
        campaign = record["Campagne"]
        adgroup = record["Ad Group"]
        top_keywords = record["Top 3 Mots-Clés"]
...
def main():
    ...
    page = st.sidebar.radio("Aller à", ["Clients","Brief Global","Google Sheet","Contextualisation","Configuration IA","Génération","Résultats"])
    if page == "Clients":
        page_clients()
